...
SimpleRouter::group(['prefix' => '/api'], function() {
    ...
    SimpleRouter::get('/user/settings', function() {
        ...
        if (!$settings) {
            ...
            echo json_encode([
                ...
                'font_family' => $settings['font_family'] ?? 'Courier New, Monaco, Consolas, monospace',
                'font_size' => (int)($settings['font_size'] ?? 16)
            ]);
        }
    });
    ...
    SimpleRouter::post('/user/settings', function() {
        ...
        try {
            ...
            $fontFamily = $input['font_family'] ?? 'Courier New, Monaco, Consolas, monospace';
            $fontSize = (int)($input['font_size'] ?? 16);
            ...
            $stmt = $db->prepare("
                ...
                (user_id, messages_per_page, timezone, theme, show_origin, show_tearline, auto_refresh, font_family, font_size)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
