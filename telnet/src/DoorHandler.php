...
class DoorHandler
{
    ...
    public function show($conn, array &$state, string $session): void
    {
        ...
        if (empty($allDoors)) {
            ...
            TelnetUtils::writeLine($conn, TelnetUtils::colorize($this->server->t('ui.telnet.doors.no_doors', 'No doors are currently available.', [], $state['locale']), TelnetUtils::ANSI_YELLOW));
        ...
        while (true) {
            ...
            TelnetUtils::writeLine($conn, TelnetUtils::colorize($this->server->t('ui.telnet.doors.invalid', 'Invalid selection.', [], $state['locale']), TelnetUtils::ANSI_RED));
            sleep(1);
        }
    }
    ...
    private function displayDoorList($conn, array &$state, array $doorList): void
    {
        ...
        TelnetUtils::safeWrite($conn, "\033[2J\033[H");
        TelnetUtils::writeLine($conn, TelnetUtils::colorize($this->server->t('ui.telnet.doors.title', '=== Door Games ===', [], $state['locale']), TelnetUtils::ANSI_CYAN . TelnetUtils::ANSI_BOLD));
        TelnetUtils::writeLine($conn, '');
        ...
        TelnetUtils::writeLine($conn, TelnetUtils::colorize($this->server->t('ui.telnet.doors.enter_choice', 'Enter number to play, or Q to return: ', [], $state['locale']), TelnetUtils::ANSI_DIM));
    }
    ...
    {
        TelnetUtils::safeWrite($conn, "\033[2J\033[H");
        TelnetUtils::writeLine($conn, TelnetUtils::colorize($this->server->t('ui.telnet.doors.launching', 'Launching {name}...', ['name' => $doorName], $state['locale']), TelnetUtils::ANSI_CYAN));
        TelnetUtils::writeLine($conn, '');
        ...
        if (empty($apiResult['success'])) {
            ...
            TelnetUtils::writeLine($conn, TelnetUtils::colorize($this->server->t('ui.telnet.doors.launch_error', 'Error: {error}', ['error' => $msg], $state['locale']), TelnetUtils::ANSI_RED));
            TelnetUtils::writeLine($conn, '');
            TelnetUtils::writeLine($conn, TelnetUtils::colorize($this->server->t('ui.telnet.server.press_any_key', 'Press any key to return...', [], $state['locale']), TelnetUtils::ANSI_YELLOW));
            $this->server->readKeyWithIdleCheck($conn, $state);
            return;
        ...
        TelnetUtils::writeLine($conn, TelnetUtils::colorize($this->server->t('ui.telnet.doors.connecting', 'Connecting to game server...', [], $state['locale']), TelnetUtils::ANSI_DIM));

        $wsSock = $this->wsConnect($wsHost, $wsPort, $wsToken);
        if ($wsSock === null) {
            TelnetUtils::writeLine($conn, TelnetUtils::colorize(
                $this->server->t('ui.telnet.doors.connect_failed', 'Could not connect to game bridge. Is the DOS door bridge running?', [], $state['locale']),
        ...
        TelnetUtils::writeLine($conn, TelnetUtils::colorize($this->server->t('ui.telnet.doors.connected', 'Connected! Starting game...', [], $state['locale']), TelnetUtils::ANSI_GREEN));
        sleep(1);
        ...
        TelnetUtils::safeWrite($conn, "\033[2J\033[H");
        TelnetUtils::writeLine($conn, '');
        TelnetUtils::writeLine($conn, TelnetUtils::colorize($this->server->t('ui.telnet.doors.returned', 'Returned from {name}.', ['name' => $doorName], $state['locale']), TelnetUtils::ANSI_CYAN));
