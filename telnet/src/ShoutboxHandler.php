...
class ShoutboxHandler
{
    ...
    public function show($conn, array &$state, string $session, int $limit = 5, bool $interactive = true): void
    {
        ...
        while (true) {
            ...
            TelnetUtils::writeLine($conn, TelnetUtils::colorize($this->server->t('ui.telnet.shoutbox.title', 'Shoutbox', [], $state['locale']), TelnetUtils::ANSI_MAGENTA . TelnetUtils::ANSI_BOLD));
            TelnetUtils::writeLine($conn, '');

            if (!$messages) {
                TelnetUtils::writeLine($conn, TelnetUtils::colorize($this->server->t('ui.telnet.shoutbox.no_messages', 'No shoutbox messages.', [], $state['locale']), TelnetUtils::ANSI_YELLOW));
            } else {
                $lineIndex = 0;
            ...
            TelnetUtils::writeLine($conn, '');
            $choice = $this->server->prompt($conn, $state, TelnetUtils::colorize($this->server->t('ui.telnet.shoutbox.menu', '[P]ost  [R]efresh  [Q]uit: ', [], $state['locale']), TelnetUtils::ANSI_YELLOW), true);
            if ($choice === null) {
                return;
            }
            ...
            $message = $this->server->prompt($conn, $state, TelnetUtils::colorize($this->server->t('ui.telnet.shoutbox.new_shout', 'New shout (blank to cancel): ', [], $state['locale']), TelnetUtils::ANSI_CYAN), true);
            if ($message === null) {
                return;
            }
            ...
            if (($response['data']['success'] ?? false) === true) {
                TelnetUtils::writeLine($conn, TelnetUtils::colorize($this->server->t('ui.telnet.shoutbox.posted', 'Shout posted.', [], $state['locale']), TelnetUtils::ANSI_GREEN . TelnetUtils::ANSI_BOLD));
            } else {
                TelnetUtils::writeLine($conn, TelnetUtils::colorize((string)($response['data']['error'] ?? $this->server->t('ui.telnet.shoutbox.post_failed', 'Failed to post shout.', [], $state['locale'])), TelnetUtils::ANSI_RED));
            }
    ...
    private function renderReadOnly($conn, array &$state, string $session, int $limit): void
    {
        ...
        TelnetUtils::writeLine($conn, TelnetUtils::colorize($this->server->t('ui.telnet.shoutbox.recent_title', 'Recent Shoutbox', [], $state['locale']), TelnetUtils::ANSI_MAGENTA . TelnetUtils::ANSI_BOLD));
