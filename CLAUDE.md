...
## Important Notes
 ...
 - **Docs viewer root-level file allowlist**: The in-app doc viewer (`src/Web/DocsController.php`) can only serve files from `docs/` and a hardcoded allowlist of root-level Markdown files. When a `docs/` page needs to link to a root-level file using a `../` path (e.g. `../REGISTER.md`), **two places** in `DocsController.php` must both contain the bare filename: (1) `$specialBases` in `resolveDocPath()` — maps the name to the real path so the viewer can read the file; (2) the `in_array` list in `rewriteLinks()` — rewrites `../NAME.md` links to `/admin/docs/view/NAME` so the viewer routes them correctly. GitHub renders `../` links correctly without any changes. Only add root-level files that are safe to expose to all authenticated users.
 - **CLI docs**: When adding or removing a script in `scripts/`, update `docs/CLI.md`.
 - **Configuration docs**: When adding a new env var to `.env.example`, update `docs/CONFIGURATION.md`.
 - **Admin daemon docs**: When adding a new admin daemon command, update the command reference in `docs/AdminDaemon.md`.
 - **BinkStream docs**: When publishing a new event through `sse_events`, update `docs/BinkStreamChannel.md`.
 - **Data model docs**: When adding new database tables or changing core entity relationships, update `docs/DATA_MODEL.md`.
...
- **ASCII quotes only in PHP catalog files:** PHP string delimiters must be ASCII single quotes (U+0027). Never use curly/smart quotes (U+2018 `'` or U+2019 `'`) as string delimiters — PHP does not recognize them and the file will fail to parse.
- **Apostrophes in catalog values must be escaped:** Any apostrophe that appears inside a PHP string value must be written as `\'`, never as a raw ASCII `'` or a curly quote U+2019. This applies to all locales, especially French (`l'`, `d'`, `qu'`) and Italian (`l'`, `all'`, `dell'`). Existing catalog entries may contain U+2019 curly quotes used as apostrophes — this is technically valid PHP (since PHP does not treat U+2019 as a string terminator) but it is fragile: when the Edit tool rewrites such a line, it can inadvertently place curly quotes in the delimiter position, breaking the file. When you edit any line that visually contains an apostrophe inside a string value, always write that apostrophe as `\'` in your replacement, regardless of how it appeared before.
- **Lint every catalog file you touch:** After writing or editing any `config/i18n/` PHP file, run `php -l config/i18n/<locale>/<file>.php` to confirm it parses cleanly before committing. Do not limit this check to Italian — the French catalog has the same apostrophe risk.
