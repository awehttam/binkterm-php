...
| `ENABLE_NNTP` | nntp_daemon | `119:8119` (plaintext + STARTTLS) + `563:8563` (implicit TLS) | Serves echoareas as newsgroups. The `binkterm` user cannot bind 119/563, so the daemon listens on 8119/8563 in-container and the override maps the standard host ports. Also turn the server on in **Admin → NNTP Server**. See `docs/NNTP.md` |
