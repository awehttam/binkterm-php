...
- BinkP session: fixed `binkp_poll` taking up to 5 minutes to complete after sending mail; the EOB exchange now terminates immediately when the remote sends `M_EOB` without a preceding `M_GOT`, instead of waiting for the full session timeout; sent packets are now also correctly deleted when the remote uses `M_EOB` as implicit acknowledgement instead of sending explicit `M_GOT` frames
