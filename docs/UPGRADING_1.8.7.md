...
- [Bug Fixes](#bug-fixes)
  ...
  - [Message Reader Header Transparency](#message-reader-header-transparency)
  - [Maximized Message Reader Gap](#maximized-message-reader-gap)
  - [TIC Replace Existing Blocked by Duplicate Hash](#tic-replace-existing-blocked-by-duplicate-hash)
  ...
  - [Service Worker PWA Deadlock (Microsoft Edge)](#service-worker-pwa-deadlock-microsoft-edge)
...
- Rich message content rendering: RIPscrip, Sixel graphics, and PCBoard @X
  colour codes are now detected and rendered inline in the message reader.
...
stream. Detection is based on RIP command lines beginning with `!|` plus
...
elements between the surrounding text segments. Sixel images in file
...
PCBoard BBS-era posts. Files with `.bbs` extensions in the file browser are
also rendered this way.

For a full list of file types that can be previewed in the file browser —
including ANSI art, MOD tracker music, PETSCII/PRG, D64 disk images, Sixel,
...
### Service Worker PWA Deadlock (Microsoft Edge)
...
`Promise.race()` with a 3-second timeout. If the lock isn't acquired in time,
fetch handlers fall back to direct network requests so the page loads normally
