...
| Point Number | Point | Suggested automatically (starting at 10 by default, configurable via `HUB_POINT_FIRST_AUTO_NUMBER` — numbers below that are reserved and never auto-assigned, but can still be entered manually here), editable |
...
- **Create a point** under any configured network (boss AKA) that isn't itself a point address, up to a configurable limit per network (`HUB_POINT_MAX_PER_USER_PER_NETWORK` in `.env`, default `1` — see `docs/CONFIGURATION.md`). The point number is allocated automatically, starting at 10 by default (configurable via `HUB_POINT_FIRST_AUTO_NUMBER` — see `docs/CONFIGURATION.md`) — numbers below that are reserved for the sysop to assign manually and are never auto-assigned to a self-service point. A Session Password and a single AreaFix Password (also used as the FileFix Password) are generated automatically. No Packet Password is generated.
