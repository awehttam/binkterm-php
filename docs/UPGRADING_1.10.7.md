...
The preview also shows when an area's description will be updated, displaying the current description struck through above the incoming one whenever the sync would actually replace it. A description is only ever replaced when the current local one is empty or an auto-generated placeholder; a real, sysop-set description is never overwritten by a hub's reply. When the hub's reply lists a different description for an area that already has its own real, sysop-set one, the preview still notes what the hub sent (without implying it will be applied), so a mismatch doesn't go unnoticed just because it won't be synced automatically.
