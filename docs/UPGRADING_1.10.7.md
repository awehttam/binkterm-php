...
Each area in the preview now has its own checkbox, with "Select All" and "Select None" buttons above the list, so you can choose exactly which areas to apply instead of it being all-or-nothing. This also gives you a way to accept a description change the automatic protection would otherwise skip: checking a row that shows a description mismatch and confirming will update that area's description to match the hub's reply, even if your local one wasn't a placeholder. An area that would create, reactivate, deactivate, or genuinely update something is pre-checked; an area that's otherwise a complete no-op — including one whose only difference is a description mismatch your own description would otherwise protect — starts unchecked, so nothing changes unless you explicitly ask it to. That same "otherwise a no-op but for a description mismatch" case is also labeled "Updated" rather than "Unchanged" in the Status column, so it stands out as something worth reviewing.
