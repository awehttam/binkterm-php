...
- Out of scope (initial rollout):
  ...
  - ~~Telnet/BinkP protocol payload text~~ â€” **Telnet daemon localization was added to scope and completed in 1.8.6.** All user-facing strings in the telnet server (shell menus, message editor, echomail/netmail browsers, polls, shoutbox, door launcher) are translated via the `terminalserver` catalog namespace.
...
- Ship incrementally by page area, not by â€œconvert everything at onceâ€.
...
### Status Update (March 6, 2026) â€” Shipped in 1.8.6
...
- Phase 5 (Hardening and Cleanup): **In Progress (advanced)**
  ...
  - Remaining: no automated Playwright smoke pass runs with the user locale set to `es` â€” secondary locale coverage is manual only.
- Telnet Daemon Localization (added to scope): **Completed**
  - All user-facing strings in the telnet server translated via the `terminalserver` catalog namespace.
  - Daemon defaults to system locale pre-login, switches to user's saved locale after login.
  - Ships with `en` and `es` catalogs (`config/i18n/<locale>/terminalserver.php`).
...
**User-facing daemons** (output seen directly by end users â€” highest priority):
- `src/TelnetServer/` â€” **Completed in 1.8.6** via `terminalserver` catalog namespace.

**Interactive sysop tools** (output seen by the person running the command):
- `scripts/user-manager.php` â€” interactive user management CLI
- `scripts/binkp_status.php` â€” status display
- `scripts/who.php` â€” who's online
- `scripts/admin_client.php` â€” admin CLI client
...
**Current Status: Deferred â€” compatibility window still open.**
...
3. Legacy `error` field retirement is tracked in Phase 7 â€” no action needed until the compatibility window is judged closed.
